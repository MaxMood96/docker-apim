...
This section defines the step-by-step instructions to build a [Rocky Linux](https://hub.docker.com/_/rockylinux/) based Docker image for WSO2 API Manager - Traffic Manager 4.7.0.
...
> The local copy of the `dockerfiles/rocky/apim-tm` directory will be referred to as `AM_DOCKERFILE_HOME` from this point onwards.
...
```
docker build -t wso2am-tm:4.7.0-rocky .
```
...
```
docker run -it -p 9443:9443 -p 9611:9611 -p 5672:5672 -p 9711:9711 wso2am-tm:4.7.0-rocky
```
...
The above wso2am-tm:4.7.0-rocky image will only be supported for the CPU architecture of your current machine. Docker buildx plugin can be used to build wso2am-tm:4.7.0-rocky image to support any CPU architecture.
...
```
docker buildx build --platform linux/amd64,linux/arm64 --load -t <DOCKER_USERNAME>/wso2am-tm:4.7.0-rocky --push .
```
...
#### 5. Run
```
docker run -it -p 9443:9443 -p 8243:8243 <DOCKER_USERNAME>/wso2am-tm:4.7.0-rocky
```
