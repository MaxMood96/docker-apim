...
This section defines the step-by-step instructions to build a [Rocky Linux](https://hub.docker.com/_/rockylinux/) based Docker image for WSO2 API Manager - Gateway 4.7.0.
...
> The local copy of the `dockerfiles/rocky/apim-gw` directory will be referred to as `AM_DOCKERFILE_HOME` from this point onwards.
...
```
docker build -t wso2am-universal-gw:4.7.0-rocky .
```
...
```
docker run -it -p 8243:8243 wso2am-universal-gw:4.7.0-rocky
```
...
wso2am-universal-gw:4.7.0-rocky
```

> In here, <TARGET_CONFIGS> refers to /home/wso2carbon/wso2am-universal-gw-4.7.0-m1/repository/conf folder of the container.

## How to build a Docker image with multi-architecture support

The above wso2am-universal-gw:4.7.0-rocky image will only be supported for the CPU architecture of your current machine. Docker buildx plugin can be used to build wso2am-universal-gw:4.7.0-rocky image to support any CPU architecture.
...
```
docker buildx build --platform linux/amd64,linux/arm64 --load -t <DOCKER_USERNAME>/wso2am-universal-gw:4.7.0-rocky --push .
```
...
#### 5. Run
```
docker run -it -p 9443:9443 -p 8243:8243 <DOCKER_USERNAME>/wso2am-universal-gw:4.7.0-rocky
```
