...
This section defines the step-by-step instructions to build an [Ubuntu](https://hub.docker.com/_/ubuntu/) Linux based Docker image for WSO2 API Manager - Gateway 4.7.0.
...
> The local copy of the `dockerfiles/ubuntu/apim-gw` directory will be referred to as `AM_DOCKERFILE_HOME` from this point onwards.
...
```
docker buildx build --platform linux/amd64,linux/arm64 --load -t <DOCKER_USERNAME>/wso2am-universal-gw:4.7.0 --push .
```
