...
This section defines the step-by-step instructions to build an [Ubuntu](https://hub.docker.com/_/ubuntu/) Linux based Docker image for WSO2 API Manager - API Control Plane 4.7.0.
...
```
docker build -t wso2am-acp:4.7.0 .
```
...
```
docker run -it -p 9443:9443 -p 9611:9611 -p 9711:9711 -p 5672:5672 wso2am-acp:4.7.0
```
...
The above wso2am-acp:4.7.0 image will only be supported for the CPU architecture of your current machine. Docker buildx plugin can be used to build wso2am-acp:4.7.0 image to support any CPU architecture.
...
#### 5. Run
```
docker run -it -p 9443:9443 -p 8243:8243 <DOCKER_USERNAME>/wso2am-acp:4.7.0
```
